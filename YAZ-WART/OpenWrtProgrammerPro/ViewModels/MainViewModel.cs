...
namespace OpenWrtProgrammerPro.ViewModels
{
    public class MainViewModel : ObservableObject
    {
        ...
        public string LicenseBadgeText
        {
            get
            {
                var lic = _licenseValidator.ActiveLicense;
                if (lic == null) return "🛡️ غير مرخص";
                
                DateTime today = DateTime.Today;
                if (today > lic.ExpiryDate)
                {
                    int grace = lic.GracePeriodDays - (today - lic.ExpiryDate).Days;
                    return $"🛡️ ترخيص منتهي (سماح: {grace} يوم)";
                }
                else
                {
                    int remaining = (lic.ExpiryDate - today).Days;
                    return $"🛡️ ترخيص نشط: {lic.CustomerName} ({remaining} يوم)";
                }
            }
        }
        ...
        {
            var licenseInfoWindow = new LicenseInfoWindow
            {
                Owner = Application.Current.MainWindow
            };
            licenseInfoWindow.ShowDialog();
